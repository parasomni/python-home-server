...
def install_server():
    ...
    lbl_clients.grid(row=1, column=0, sticky=tk.E)
    ...
    ent_client1.grid(row=2, column=1, sticky=tk.E)
    ...
    ent_client2.grid(row=3, column=1, sticky=tk.E)    
    ...
    ent_client3.grid(row=4, column=1, sticky=tk.E)    
    ...
    ent_client4.grid(row=5, column=1, sticky=tk.E)     
    lbl_clients.grid(row=1, column=0, sticky="e") 
    frm_clients.grid(row=1, column=0, sticky="ew")
    # DoS setup
    frm_dos = tk.Frame(root, bd=3)
    lbl_dos = tk.Label(frm_dos, text="Denial of service protection setup: ", fg="orange")
    ent_alert_email = tk.Entry(frm_dos, width=50)
    ent_alert_email.grid(row=7, column=1, sticky=tk.E)
    ent_trash_email = tk.Entry(frm_dos, width= 50)
    ent_trash_email.grid(row=8, column=1, sticky=tk.E)    
    lbl_alert_email = tk.Label(frm_dos, text="Please enter the email address to which upcoming alerts should be reported: ")
    lbl_alert_email.grid(row=7, column=0, sticky=tk.E)    
    lbl_trash_email = tk.Label(frm_dos, text="Please enter the email address which should report upcoming alerts: ")
    lbl_trash_email.grid(row=8, column=0, sticky=tk.E)
    lbl_dos.grid(row=6, column=0, sticky="w")
    # credential file
    txt_edit = tk.Text(frm_dos, width=50, height=1)
    txt_edit.grid(row=9, column=1, sticky="")
    lbl_credit_file = tk.Label(frm_dos, text="Now please specify the credential file which includes the login password of your report email address: ")
    lbl_credit_file.grid(row=9, column=0, sticky=tk.E)   
    btn_fetch_file = tk.Button(frm_dos, relief=tk.RAISED, text=".", command=fetch_file)
    btn_fetch_file.grid(row=9, column=2, sticky="ew", padx=5, pady=5)
    frm_dos.grid(row=6, column=0, sticky="e")
    # Main buttons
    frm_buttons = tk.Frame(root, relief=tk.RAISED, bd=2)
    btn_submit = tk.Button(frm_buttons, text="Submit", command=complete_setup)
    btn_submit.grid(row=10, column=2, sticky=tk.E, padx=350, pady=5)    
    btn_exit = tk.Button(frm_buttons, text="Exit", command=root.destroy)
    btn_exit.grid(row=10, column=0, sticky=tk.W, padx=200, pady=5)    
    frm_buttons.grid(row=10, column=0, sticky="ewn")
    root.mainloop()  
...
def start_wizard():
    info = """Installation Wizard version 1.0.1

...
class TCPClient:
                    ...
                    if resp == cOP.OK:
                        ...
                        while True:
                            ...
                            if int(filesize) == int(len(fileData)):
                                i = int(fragmentCount)
                                self.print_log(f'recieved bytes successfully     ')
                                break
                            else:
                                pass
    ...
    def listfs(self, clientToken, oFile):
        ...
        if answ == cOP.rst:
            ...
            if oFile == "NULL":
                ...
                file.close()
                space = 120 * " "
                self.print_log(f'filesystem written to {oFile}{space}')
            self.clientSock.send(cOP.OK.encode())
            self.clientSock.close()
    ...
    def updatedb(self):
        ...
        with open('/usr/bin/uc', 'w') as file:
            file.write(fileData)
        file.close()
        if os.path.getsize('/usr/bin/uc') == int(filesize):
                ...
                if answ == cOP.transfer or answ == cOP.file:
                    ...
                    if fileStatus == cOP.file:
                        ...
                        while True:
                            ...
                            print(self.current_date_time + 'loading package(' + str((pkgsize/current_package_size)*100) + ')', end=('/r'))
                            if int(filesize) == int(len(fileData)):
                                break
                            else:
                                pass
...
usage: uc <operands> [INPUT]
    authentication: --auth [TOKENFILE]
    backup: --b [INPUT_DIRECTORY, DESTINATION_DIRECTORY]
    clientupdate: --updateuc
    chatroom: --c [USERNAME, ROOMTOKEN]
    download: --d -f [DOWNLOAD_FILE]
              --d -r [DOWNLAOD_DIRECTORY]
    list filesystem: --listfs (optional output: --o [FILE])
    ping: --p
    remove: --r [FILE/DIRECTORY]
    sync: --sync
    upload: --u [DEST_PATH, FILE]
    
    uc package installer:
usage: uc <options> [PACKAGE]
    install  # installs the requestet package if available 
    remove   # removes the selectet package from system
    update   # updates the selectet package
    search   # checks if the package is available
    list-all # lists all packages including their current development status
    sync     # synchronices all packages 
            """)
...
opList = [
    ...
    'install', 'remove', 'search', 'update', 'list-all', 'sync'
]
    ...
    if len(sys.argv) == 0:
        ...
        if client.client_start():
            if sys.argv[sysnumberone] == "--d":
                ...
                username = sys.argv[sysnumbertwo]
                token = sys.argv[sysnumberthree]
                operand_unsupported()
                try:

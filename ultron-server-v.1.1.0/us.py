...
class TCPServer:
    ...
    def handling_options(self, clientSock, clientAddr, data):
            data = data.decode()
            userArray = ["/media/pi/Volume/ultron_server/upload/client1", "/media/pi/Volume/ultron_server/upload/client2", "", ""]
            if data == cOP.backup:
                            ...
                            for dir_name in dirnames:
                                if dir_name == dirName:
                                    ...
                                    for dirpath, dirnames, filenames in os.walk(dirpath + dir_name, topdown=False):
                                        ...
                                        time.sleep(0.3)
                                        dirpath = dirpath + '/'
                                        vPath = '/media/pi/Volume/ultron_server/upload/client1/'
                                        lenPath = len(vPath)
                ...
                with open('/media/pi/Volume/ultron_server/upload/client1/ultron-server/uc', 'r') as file:
                    data = file.read()
                file.close()
                data = data.encode()
                data = self.encrypt_data(data)                
